...
`ViewController.swift.template` routes its navigation title and empty-state copy
...
- `app-shell/` — rendered exactly once, by `/start`, to bootstrap the very first
  ...
  combos follow (§3.6). `/new-feature` never touches `app-shell/` again except to
  insert a new factory method at `AppCoordinator`'s marked insertion point.
