...
| **MVVM** *(default)* | `<Name>View.swift`, `<Name>ViewModel.swift`, `<Name>UseCase.swift`, `<Name>Repository.swift`, `<Name>Service.swift` | View binds to an `ObservableObject`/`@Observable` `ViewModel`; `ViewModel` never touches networking/persistence directly. |
...
└─────────┬───────────┘
          ...
          ▼
┌────────────────────┐
│      UseCase          │  pure Swift, no networking/UI imports
└─────────┬───────────┘
          │
          ▼
┌────────────────────┐
...
- Business layer (UseCase/Interactor): one operation, pure Swift, no UI/networking imports — depending on the data layer's *protocol*, never its implementation (§8.2).
...
- **Rules `/new-feature` and `/add-module` enforce:**
  - Every `Service`/`Repository`/`UseCase` is generated as a **protocol plus a concrete implementation**, with the consumer depending on the protocol. This is what makes §4.1's "real, passing unit test against a fake dependency" possible at all — the fake is a second conformance, not a mocking framework.
