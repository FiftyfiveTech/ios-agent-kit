...
- *Recommended:* an `AppEnvironment` type is the right shape **once a second configuration exists**, not before — until then the `Info.plist` key plus `RequestBuilder`'s injected `baseURL` already is the seam, and generating a wrapper with one case is the speculation §8.2 warns against. `/start` records it as a `TODO.md` entry instead of generating it.
