...
- **A view used by one screen stays with that screen; only a second consumer makes it shared.** Three shapes, in order, and a view moves up only when it earns it:
  ...
  3. **Its own child-screen folder** — `Features/Home/Detail/` with that screen's full layer set, generated by `/new-feature Home/Detail` — once it is a second *screen* rather than a component: it has its own data source, its own navigation entry, or its own tests.
