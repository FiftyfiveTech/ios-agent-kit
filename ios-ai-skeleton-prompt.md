...
**This applies to every combination, not only the SwiftUI ones.** A UIKit project gets `LoadingView`/`ErrorView`/`EmptyStateView` as plain `UIView` subclasses in the same `DesignSystem/SharedViews/` folder, generated by `/start` alongside the app shell, and its ViewControllers consume those instead of building a spinner and an error label inline. Same rule, same location, different concrete type — a per-feature reimplementation is the thing being prevented, and that failure mode is if anything *more* common in UIKit, where a `UIActivityIndicatorView` plus two constraints is easy enough to retype that it gets retyped in every screen.

The general rule this is the first instance of: **a view used by more than one screen belongs in `DesignSystem/SharedViews/`, at every tier and in both UI frameworks.** When `/new-feature` generates a screen that needs a component another screen already has — a styled primary button, a labelled row, a card container — it consumes the shared one or promotes the existing one into `SharedViews/`; it does not copy it into the new feature folder. This is deliberately a stronger stance than §8.2's "don't abstract on speculation," and the two do not conflict: §8.2 governs *protocol* boundaries invented ahead of a second conformance, whereas a second screen rendering the same component is the second use case already having arrived. From T2 onward these views live in the shared module and are `public`; per-app visual differences go through the theme override layer (§3.11), never through a forked copy of the view.
...
- **`AppEnvironment` is rendered here, on first use** — from `Scripts/templates/config/AppEnvironment.swift.template`, into Core at the tier-correct path (the same table §1.7a uses for `PersistenceController`: `<app>/Core/Configuration/` at T1, `Packages/Core/Sources/Core/Configuration/` at T2, the `role: "core"` module's `Sources/<Name>/Configuration/` at T3). It lives in the shared module, not the app, so a feature in any module can read configuration without the app threading it down — `Bundle.main` resolves to the app bundle even from inside a package or framework. That same fact makes it untestable from a package test bundle, which is why the type takes an injectable lookup defaulting to `Bundle.main`.
...
- **`API_BASE_URL` deliberately stays where it is**, on its composition-root path into `RequestBuilder`'s `baseURL` (§3.7). Two readers for one key with two different failure messages is worse than one reader in a less tidy place; folding it into `AppEnvironment` is a named follow-up (§10), not a side effect of adding an unrelated key.
...
- **Decided: `AppEnvironment` exists, and `/add-secret` (§4.10) materializes it on first use — `/start` still never generates one.** The old position (a wrapper is the right shape only once a second configuration exists) was right about the threshold and wrong about who crosses it: the moment a project adds a *second key*, the alternative to a shared reader isn't "the `Info.plist` key plus `RequestBuilder`'s injected `baseURL`", it's a `Bundle.main.object(forInfoDictionaryKey:)` call somewhere inside a feature — which is the thing §3.7 and `docs/ai/ui_rules.md` both forbid, and which no amount of prose prevents if the template offers nothing to use instead. So the file is created by the Skill that first needs it, the same way `docs/PERMISSIONS.md` is created by the first `/add-permission`, and a one-key project still carries no wrapper. It lives in **Core**, not the app, because a shared module can't read config the app hasn't injected otherwise, and it takes an injectable lookup defaulting to `Bundle.main` so a package test target can exercise it at all.
...
- [ ] `Scripts/add_secret.sh` and `Scripts/templates/config/AppEnvironment.swift.template` (§4.10) — the second is rendered by `/add-secret` on first use, never by `/start`
...
- [ ] `DesignSystem/SharedViews/` populated for the project's UI framework — SwiftUI views for the SwiftUI combos, `UIView` subclasses for the UIKit ones (§3.6)
...
- **Two readers for configuration, not one.** `/add-secret` renders `AppEnvironment` in Core with typed accessors, but `API_BASE_URL` keeps its original path through the app shell's own `apiBaseURL` reader into `RequestBuilder` (§8.3). Consolidating them means editing all four app-shell templates and picking one set of failure messages — worth doing, deliberately, as its own change; doing it halfway would leave two readers with two different messages for the same key, which is strictly worse than the current split.
