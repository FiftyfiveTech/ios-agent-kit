...
**Both open with a numbered quick start, in plain language** — install the tools, clone, run `/start`, answer the questionnaire, fill in `Secrets.xcconfig` (with the `https:/$()/host` form, since that trap is hit on the first build), open and run. Six steps or so, commands spelled out in `README.md` and one line each in `docs/ONBOARDING.md`, before any explanation of how the template works. Someone who reads only that list must be able to get to a running app; everything after it is reference, and the two files point at each other rather than repeating each other — `README.md` owns the model tiers and the full limitations list, `docs/ONBOARDING.md` owns the day-two material (secrets, when to regenerate, localization) and summarizes rather than restates. A reader who abandons either file halfway is the failure mode these two rules exist to prevent.
...
0. **Resolve the target path and copy the template's own files into it.** `/start` takes one optional argument, a path (§0).
   - No path → the target is the current directory; nothing to copy (it's either already the template's own clone, or the developer copied the files in by hand already). **Passing a path is the recommended way to run `/start`, and the no-path case needs one confirmation before anything is written:** when the current directory still looks like an un-renamed template checkout (`CLAUDE.md.template`, `Scripts/templates/`, the build spec and overview present, no `ios-skeleton.config.json`, remote still the template's), state the resolved absolute path and ask whether to scaffold here or into a subdirectory. This is not detectable as an error — cloning the template as the new project's root is a supported start and looks identical — so it's a confirmation, never a refusal, and it is not asked when a path was given.
...
2. Ask the Setup Questionnaire (§2.2) in one batched interaction, not one question per turn. **Presenting it is mandatory on every first run** — stated preferences in the invocation pre-fill the list, they never skip showing it; see §2.2.
...
8. Wire `.githooks/pre-commit` via `core.hooksPath` — on Path 3 (step 13), only after that step's adoption dry-run passes, or the developer explicitly accepts report-only mode.
...
**Always present the full list.** A first run shows all eleven askable questions before anything is written to disk — even when the invocation already named an architecture, even when the developer opens with "just use the defaults", and even when the repo makes the answers look obvious. Preferences stated up front pre-fill the list; they don't replace showing it, because a developer who never saw Q4 or Q6 doesn't know a decision was made for them, and every Skill afterwards reads those answers back as if they had been chosen. The only carve-out is Path 3's Q8 (§2.1 step 13), which is read off the existing project's build settings rather than asked — one fewer question, not a skipped step. Ask as one consolidated numbered prompt with each recommended default shown inline; a picker tool can carry a subset of the picklist questions as an aid, never the questionnaire in place of the written list. Accepting the defaults stays a single reply, and Q12 is the one answer the developer must supply.

Ask it like this:

> "Before I scaffold your app, here are the decisions I need — every one has a recommended default, so you can answer inline, or reply 'use the recommended defaults' and just give me question 12:
...
The general rule this is the first instance of: **a view used by more than one screen belongs in `DesignSystem/Views/`, at every tier and in both UI frameworks.** When `/new-feature` generates a screen that needs a component another screen already has — a styled primary button, a labelled row, a card container — it consumes the shared one or promotes the existing one into `DesignSystem/Views/`; it does not copy it into the new feature folder. This is deliberately a stronger stance than §8.2's "don't abstract on speculation," and the two do not conflict: §8.2 governs *protocol* boundaries invented ahead of a second conformance, whereas a second screen rendering the same component is the second use case already having arrived. From T2 onward these views live in the shared module and are `public`; per-app visual differences go through the theme override layer (§3.11), never through a forked copy of the view. **At T3 the promotion trigger is a second *consumer*, not a second call site:** a component library that exists for exactly one app — a SwiftUI migration target, a new product — may sit in that app until a second app needs it, and moving it down is then a deliberate module move (§3.10), not a copy left behind in both places. What is never acceptable is the same component existing in two apps at once.
...
2. **Grouping** — flat `<module>/Features/<Name>/` by default; ask with the concrete alternative whenever the request implies a section or flow, whenever the resolved module's feature directory already has group folders, and — the case a flat default gets wrong most often — **whenever a feature already exists that the new one plainly belongs to**: with `Features/Contacts/` present, `AddContact`/`Favorites`/`ContactDetails` are proposed as `Features/Contacts/<Name>/`, not as three more top-level folders. §3.2 already allows a screen folder to be a leaf and a group at once, so the parent keeps its own layer files and nothing existing moves. Nesting is directory placement only — type names, navigation registration and `Models/<Name>Models.swift` stay in one flat namespace the path doesn't scope, so a child screen's name must be unique app-wide (`ContactDetails`, not `Details`). There is no fixed `Features/` path to inspect until step 1 is settled.
...
| `docs/GIT_CONVENTIONS.md` | shipped as-is | Feature-based commits, optional branch naming, and a suggested message format the team is free to replace — nothing here is enforced |
...
- **Commit message format is deliberately not enforced (§10).** `docs/GIT_CONVENTIONS.md` ships a suggested format, but no hook checks it and none should: message conventions differ per team and per client, and a template that binds a project to one house format is imposing a decision that isn't its to make. A team that wants the format mechanically checked adds its own `commit-msg` hook; the template's job is to have written the convention down, not to police it.
...
- *Observed in production, and the reason the layers look asymmetric:* that repo is **mid-migration from UIKit to SwiftUI, one app at a time.** Originally both apps drew common views and whole screens from the components framework, in UIKit, keeping only app-specific ones locally. When the consumer app was chosen as the SwiftUI migration target, its new SwiftUI component library and its theming went into **that app**, not the framework — because it had exactly one consumer. The pro app stays on UIKit against the framework's original shared layer, unchanged, and the consumer app's own not-yet-migrated features still use that same UIKit shared code. So the framework's `Views/` holding shared *screens and flows* (address selection, barcode scanner, device lists) while a component library sits inside one app is not drift: it is the old layer and the new layer coexisting, with the new one staged in its only consumer.

  **Two rules follow, and this template holds both.** First, §3.6's promote-on-second-use rule is about the second *consumer*, not the second call site: a component library with one app using it is correctly app-local, and it moves down into the shared module when the second app migrates — which is a `/add-module`-shaped move (§3.10), planned, not a copy. Second, a shared module may legitimately carry **two component sets during a migration**, one per UI framework, rather than one being converted in place; the template's own one-combo-per-project answer (Q2/Q3) describes a project's *new* code, and §2.1's Path 3 exists because an adopted repo's existing code is whatever it already is.
...
- *Observed in production:* commits carry the tracker ticket ID and land via PR. Encoded in `docs/GIT_CONVENTIONS.md` as a recommendation, not a check (§7, §10) — the observation holds for that codebase, but whether a team's tracker issues ticket IDs at all, and what shape its messages take, is that team's call.
...
- [ ] `.swiftlint.yml`, `Scripts/lint.sh`, `Scripts/check_hardcoded_colors.sh`, `Scripts/check_strings.sh` (§8.5), `Scripts/check_secrets.sh` (§8.3), `Scripts/generate_strings.sh` taking a module argument, `.githooks/pre-commit` (§7)
...
- [ ] `CLAUDE.md.template`, `README.md`/`docs/ONBOARDING.md` describing the template itself, each opening with §1.2's numbered quick start (§1.2)

### 9.2 Phase B — what a successful `/start` run must produce (build this expectation into the Skill itself)

- [ ] The Setup Questionnaire was **presented in full before anything was written** — all eleven askable questions with their defaults shown, in one consolidated prompt (§2.2), never inferred from the invocation or silently defaulted
...
- ~~The questionnaire could be skipped when the developer stated preferences up front.~~ **Resolved:** §2.2 said to tell the developer they could say "use the recommended defaults" and the Skill would "fill in the rest", which read as a licence to not display the questions at all — an invocation like `/start MyApp with MVVM + SwiftUI` could scaffold without the developer ever seeing Q4 or Q6, whose answers every later Skill reads back as if they had been chosen deliberately. Presenting the full list is now mandatory on every first run (§2.1 step 2, §2.2, §9.2's checklist): stated preferences pre-fill it, accepting all the defaults is still one reply, and Path 3's Q8 remains the only question removed rather than skipped. The re-run flow (§2.4) is unchanged — it prints the recorded answers and asks nothing unless something is missing or conflicts.
...
- **`docs/GIT_CONVENTIONS.md` is documentation only, not tool-enforced — deliberately, and now settled.** This was briefly resolved the other way: a `.githooks/commit-msg` hook checked the `(<TICKET-ID> )?<type>: <summary>` format mechanically. That hook has been removed and will not come back. Commit conventions differ per team and per client, so binding every project generated from this template to one house format was the template overreaching — it isn't the kit's decision to make. The convention is still written down for a team that wants it; enforcing it, and picking a different format entirely, are both the project's call. What was always convention-only stays so: whether a commit is genuinely feature-based/one-coherent-change (§7's other bullets) is a review concern no hook can check.
