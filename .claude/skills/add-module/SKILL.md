...
2. **Run the deterministic half:**
   ...
   strings.
3. **Wire it into each consumer's spec — this is your job, not the script's.**
...
4. **Enforce the dependency rule.** A module may not depend on an app. A
...
5. **Record it.** Add the module, its kind, and its consumers to
